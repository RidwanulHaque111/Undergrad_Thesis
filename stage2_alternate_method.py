...
for i in range(num_bats):
    model.fit(np.expand_dims(best_bat_positions[i], axis=0), np.expand_dims(y_train[i], axis=0), batch_size=32, epochs=10, callbacks=[lr_scheduler, early_stopping, checkpoint], validation_data=(X_test, y_test))
